...
def proposal1(
    ...
    resid = kwargs.get('resid', np.ones(m))
    # Huber reweighting
    w = np.minimum(1, tau / np.abs(resid))

    scale = np.sqrt(m / k)
    S = csr_array((np.ones(k) * scale * w[i], (r, i)), shape=(k, m))
...
def proposal2(
    ...
    # Huber reweighting
    w = np.minimum(1, tau / np.abs(resid))
    i = rng.choice(m, size=k, p=w / np.sum(w))
